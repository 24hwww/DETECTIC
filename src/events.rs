...
mod tests {
    ...
    // --- STEP 2 tests: active transitions ---

    fn dev_active(mac: &str, active: Option<&str>) -> Device {
        Device {
            hostname: Some("moto-g42".into()),
            ip: Some("192.168.0.20".into()),
            mac: Some(mac.into()),
            rssi: Some(50),
            standard: Some("ac".into()),
            onemesh_stack: None,
            assoc_time: None,
            radio_mac: Some("00:11:22:33:44:55".into()),
            source: Some("wifi".into()),
    ...
    #[test]
    fn step2_active_1_to_0_currently_produces_updated_not_disconnected() {
        // Snapshot A: active=1, Snapshot B: active=0
        // The legacy diff_to_events produces DeviceUpdated (not DeviceDisconnected).
        // The canonical DeviceDisconnected event is generated by the temporal
        // engine (temporal.rs) when the device is filtered out of device_obs
        // in service.rs. This test documents the legacy path behavior.
        let before = dev_active("AA:BB:CC:00:00:01", Some("1"));
        let after = dev_active("AA:BB:CC:00:00:01", Some("0"));
        let diff = MapDiff {
            added: vec![],
            removed: vec![],
            changed: vec![(before, after)],
        };
        let events = diff_to_events(&diff, 1000, |id| pseudo(id));
        assert_eq!(events.len(), 1);
        assert_eq!(events[0].kind, EventKind::DeviceUpdated);
        assert!(events[0].changed_fields.contains(&"active".to_string()));
    }

    #[test]
    fn step2_active_0_to_0_produces_zero_events() {
        let before = dev_active("AA:BB:CC:00:00:01", Some("0"));
        let after = dev_active("AA:BB:CC:00:00:01", Some("0"));
        let diff = MapDiff {
            added: vec![],
            removed: vec![],
        ...
        let events = diff_to_events(&diff, 1000, |id| pseudo(id));
        assert!(events.is_empty(), "active=0→0 should produce 0 events");
    }

    #[test]
    fn step2_active_1_to_1_produces_zero_events() {
        let before = dev_active("AA:BB:CC:00:00:01", Some("1"));
        let after = dev_active("AA:BB:CC:00:00:01", Some("1"));
        let diff = MapDiff {
            added: vec![],
            removed: vec![],
            changed: vec![(before, after)],
        ...
        let events = diff_to_events(&diff, 1000, |id| pseudo(id));
        assert!(events.is_empty(), "active=1→1 should produce 0 events");
    }
